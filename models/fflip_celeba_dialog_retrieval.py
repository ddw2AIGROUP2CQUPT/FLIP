...
class CelebA_Dialog_Retrieval(nn.Module):
    def __init__(self,                 
                 text_config = '/home/ubuntu/lxd-workplace/LYT/FFLIP/itc_itm_mm/configs/bert_config.json', 
        ...
        self.t_proj = nn.Linear(text_width, embed_dim)
        self.itm_proj = nn.Linear(text_width, 2)
        
        self.model_pairs = [[self.visual_encoder,self.visual_encoder_m],
                            [self.v_proj,self.vision_proj_m],
                            [self.text_encoder,self.text_encoder_m],
                            [self.t_proj,self.text_proj_m],
                           ]       
        self.copy_params()
    ...
    def forward(self, image, caption, alpha, idx):
        ...
        image_feat = F.normalize(self.v_proj(image_output.last_hidden_state[:,0,:]), dim=-1)
        text_feat = F.normalize(self.t_proj(text_output.last_hidden_state[:,0,:]),dim=-1)
...
def celeba_dialog_retrieval(pretrained='', **kwargs):
    model = CelebA_Dialog_Retrieval(**kwargs)
    if pretrained:
