...
from models.fflip_celeba_dialog_retrieval import celeba_dialog_retrieval
from models import utils
from eval.dialog_eval import evaluation, itm_eval
...
def main(args, config):
    ...
    # 随机抽样获取1%的测试数据
    # test_data_length = int(len(test_dataset) * 0.01)
    # random.seed(42)  # 设置随机种子以确保可重复性
    # random_indices = random.sample(range(len(test_dataset)), test_data_length)
    # test_dataset = [test_dataset[i] for i in random_indices]
    if args.distributed:
    ...
    model = celeba_dialog_retrieval(pretrained=config['pretrained'], vit=config['vit'], queue_size=config['queue_size'])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config', default='/home/ubuntu/lxd-workplace/LYT/FFLIP/itc_itm_mm/configs/celeba_dialog_retrieval.yaml')
    parser.add_argument('--output_dir', default='outputs/celeba_dialog_retrieval')
    ...
    parser.add_argument('--distributed', default=True, type=bool, help='whether to use distributed mode to training')
