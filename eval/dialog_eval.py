...
def evaluation(args, model, data_loader, device, config):
    ...
    for i in range(0, num_text, text_bs):
        ...
        text_embed = F.normalize(model.t_proj(text_feat.last_hidden_state[:,0,:]), dim=-1)
    ...
    for i, (image, img_id) in enumerate(data_loader): 
        ...
        image_embed = F.normalize(model.v_proj(image_feat[:,0,:]), dim=-1)
    ...
    for i, sims in enumerate(metric_logger.log_every(sims_matrix[start:end], 50, header)):
        ...
        score = model.itm_proj(output.last_hidden_state[:, 0, :])[:, 1]
